...
const MONSTER_ROOM_WEIGHT := 4.0
const SHOP_ROOM_WEIGHT := 2.5
const CAMPFIRE_ROOM_WEIGHT := 4.0
const SUPRISE_ROOM_WEIGHT := 2.5

var random_room_type_weights = {
	Room.Type.MONSTER: 0.0,
	Room.Type.CAMPFIRE: 0.0,
	Room.Type.SHOP: 0.0,
	Room.Type.SUPRISE: 0.0
}
...
func _would_cross_existing_path(i: int, j: int, room: Room) -> bool:
	...
	var right_neigbour: Room
	
	if j > 0:
		left_neighbour = map_data[i][j - 1]
		
	if j < MAP_WIDTH - 1:
		right_neigbour = map_data[i][j + 1]		

	if right_neigbour and room.column > j:
		for next_room: Room in right_neigbour.next_rooms:
...
func _setup_random_room_weights() -> void:
	...
	random_room_type_weights[Room.Type.SUPRISE] = MONSTER_ROOM_WEIGHT + CAMPFIRE_ROOM_WEIGHT + SHOP_ROOM_WEIGHT + SUPRISE_ROOM_WEIGHT
	
	random_room_type_total_weight = random_room_type_weights[Room.Type.SUPRISE]	
...
func _set_room_randomly(room_to_set: Room) -> void:
	...
	var campfire_on_6 := true
	
	var type_candidate: Room.Type	
	
	while campfire_below_4 or consecutive_campfire or consecutive_shop or campfire_on_6:
		...
		campfire_on_6 = is_campfire and room_to_set.row == 5
